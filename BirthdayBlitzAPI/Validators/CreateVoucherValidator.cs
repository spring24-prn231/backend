...
namespace BirthdayBlitzAPI.Validators
{
    public class CreateVoucherValidator : AbstractValidator<CreateVoucherRequest>
    {
        ...
        public CreateVoucherValidator(IOrderService orderService)
        {
            ...
            RuleFor(x => x.Code).MaximumLength(16).WithMessage("Code không được vượt quá 16 ký tự")
                .Matches("^[a-zA-Z0-9]*$").WithMessage("Code chỉ được chứa ký tự và số");
            ...
            RuleFor(x => x.MaximumValue).GreaterThanOrEqualTo(0).WithMessage("MaximumValue phải lớn hơn 0đ")
                .PrecisionScale(1, 20, false).WithMessage("MaximumValue không hợp lệ, chỉ được chứa tối đa 1 chữ số sau dấu thập phân");
